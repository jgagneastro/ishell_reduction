function Queryvizier, catalog, target, dis, VERBOSE=verbose, CANADA = canada, $
               CONSTRAINT = constraint, ALLCOLUMNS=allcolumns, SILENT=silent, COUNT=count, OUTCOL=outcol
;+
; NAME: 
;   QUERYVIZIER
;
...
;    PRINT, 'rspFn= ', rspFn
;    
;    if rspCode eq 18 then goto, RETRY
;    goto, RETRY
;    stop
;    
;  ENDIF
...
; special characters in the constraint string.
 ...
 if targname then $
  ...
  if keyword_set(outcol) then queryURL = queryURL + '&-out='+outcol
  if keyword_set(count) then queryURL = queryURL + '&-out=RAJ2000'
  if keyword_set(verbose) then message,queryurl,/inf
  
  Result = -1
  for kk=0L, 999L do begin
    for jj=0L, 999L do begin
      RETRY:
;      Result = webget(QueryURL,/http10, silent=silent)
      
      text = ourl_get(QueryURL)
;      ourl = obj_new('IDLnetURL',timeout=1d7)
;      text = !NULL
;      text = ourl->get(/string_array,url=QueryURL)
      if ~keyword_set(text) then stop
      Result = {text:text}
      if ~isa(result,'struct') then $
        print, ' Download failed, retrying...' else $
        break
      wait, 1
      ourl.delete ;TEST
    endfor
  ...
  ; Find the vector tags (Format begins with a number) and remove them 
   ...
   for i=0,Ntag-1 do begin
     ...
     case strmid(fmt[i],0,1) of 
       ...
       else: cval = 'continue';message,'ERROR - unrecognized format ' + fmt[i]
       
     endcase
     ;Fix the Long64 case
     if strmid(fmt[i],0,1) eq 'I' and val[i] GE 10 then $
      cval = long64(0)
     if size(cval,/type) eq 7L then $
       if cval eq 'continue' then continue
     if i EQ 0 then   info = create_struct(colname[0], cval) else begin
       ...
       if where(tag_names(info) eq strupcase(colname[i])) ge 0 then $
          colname[i] = colname[i] + '_2'
     ;info =  create_struct(info, colname[i],cval)
     info =  create_struct(temporary(info), colname[i],cval)
     endelse
   endfor
   if size(cval,/type) eq 7L then $
    if cval eq 'continue' then continue
   if ntag gt n_tags(info) then continue
   break
 endfor
 
  i0 = max(lcol) + 4  
  if i0 GT (N_elements(t)-1) then begin 
       if ~keyword_set(silent) then message,'No sources found within specified radius',/INF
       return,-1
  endif
