Pro viewfits, imi_in, hdri, COLOR=color, ZSCALE=zscale, COLCUBE=colcube, SAVE=save, LOG=log, REGION=region, $
              CMAP=cmap, COMMANDS=commands, ZOOM=zoom, ASTRXY=astrxy, NANCOLOR=nancolor, TOFIT=tofit, $
              FILENAME=filename, SECOND_REGION=second_region
  ...
  ;         de loader les regions sur l'image DS9 (avec les coordonnees physiques). La variable COLOR (dans la structure) permet de choisir la couleur
  ;         des cercles, WIDTH leur epaisseur, DASHED de les rendre pointilles.
...
;  endif
  ...
  if keyword_set(save) then begin
    if ~isa(save,'string') then message, ' Le keyword SAVE doit contenir le chemin pour la sauvegarde'
    sc = ' -zoom to fit -saveimage png '+pathfile(save)
    cible_dir = file_dirname(save)+path_sep()
    if cible_dir eq '' then cd, current=cible_dir
  endif else sc = ''
  if keyword_set(tofit) then sc = ' -zoom to fit'
  if keyword_set(color) and ~isa(im,'list') then message, ' Voir les instructions au debut du code pour /COLOR !'
  ...
  if keyword_set(filename) then $
    fname0 = filename+'_'+curcompdate(/path) else $
  ...
  if keyword_set(region) then begin
    ...
    if max(strlowcase(tags) eq 'dashed') eq 1 then $
      dashed = 1 else dashed = 0
    ...
    printu, regionfile, 'global color='+regcolor+' dashlist=8 3 width='+strtrim(width,2)+' font="helvetica 10 normal" select=1 highlite=1 dash='+strtrim(dashed,2)+' fixed=0 edit=1 move=1 delete=1 include=1 source=1'
  ...
  if keyword_set(second_region) then begin
    tags = tag_names(second_region)
    nel = n_elements(second_region.X)
    if max(strlowcase(tags) eq 'color') eq 1 then $
      regcolor = strlowcase(second_region.COLOR) else regcolor = 'green'
    if max(strlowcase(tags) eq 'width') eq 1 then $
      width = fix(second_region.WIDTH) else width = 1
    if max(strlowcase(tags) eq 'r') eq 1 then $
      radius = float(second_region.R) else radius = (intarr(nel)+5)
    if n_elements(radius) eq 1 then radius = replicate(radius[0], nel)
    if max(strlowcase(tags) eq 'format') eq 1 then $
      format = strlowcase(second_region.FORMAT) else format = 'physical'
    radunit = ''
    if format eq 'fk5' then radunit = '"'

    bad = where(~finite(second_region.X) or ~finite(second_region.Y), nbad)
    if format eq 'physical' then begin
      second_region.X += 1
      second_region.Y += 1
    endif
  ...
  if keyword_set(colcube) then begin
    ...
    ;On ouvre ds9
    commands = ['cd "'+tmpfolder+'"', $
                '"'+ds9path+'" -rgbcube "'+name+'"'+sc+addc+zmcm+rcmd+supcommands]
