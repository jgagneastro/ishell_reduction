...
;
;  $Id: mpfit2dfun.pro,v 1.10 2007/12/15 14:28:27 craigm Exp $
;-
...
function mpfit2dfun_eval, p, dp, _EXTRA=extra
  ...
  ;J.GAGNE Considering weights=0 as non-data
  if n_elements(wts) gt 0 then begin
    wts1d = reform(wts, n_elements(wts))
    bad = where(wts le 0, nbad)
    if nbad ne 0L then remove, bad, result
  endif
...
function mpfit2dfun, fcn, x, y, z, err, p, WEIGHTS=wts, FUNCTARGS=fa, $
                   ...
                   quiet=quiet, ERRMSG=errmsg, NOP=nop, _EXTRA=extra
