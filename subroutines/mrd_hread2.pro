pro mrd_hread2, unit, header, status, SILENT = silent, FIRSTBLOCK = firstblock, $
...
; them to be truncated.   Issue a warning but continue if problems are found.
  ...
  w = where(strmid(block, 0, 8) eq 'END     ', Nend)
      ...
      if n_elements(header) gt 1d4 then break
  endwhile
...
error_return:
  ...
  errmsg = 'END Statement not found in FITS header'
        if printerr then message, 'ERROR ' + errmsg, /continue
  return
end
