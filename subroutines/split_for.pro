...
pro split_for, start, finish, varnames=varnames, nsplit=nsplit, $
  ...
  ;fluff='x123123'
  fluff = 'x'+strtrim(long(abs(randomn(getseed())*1d6)),2)
  ...
  if n_elements(debug) NE 0 AND x_chkfil('simple_batch_451451', /silent) EQ 0 then begin
    ...
    if n_elements(debug) EQ 0 then begin
      obridge=obj_new("IDL_IDLBridge", output='toto.log')

      if nsplit GT 1 then begin
        for i=1, nsplit-1 do begin
          obridge=[obridge, obj_new("IDL_IDLBridge", output='toto.log')]
        endfor
      endif
    ...
    ;for j=0L, nsplit-1L do $
    ;  file_delete, 'p'+strtrim(i,2)+fluff+'batch.pro'
  endelse
end